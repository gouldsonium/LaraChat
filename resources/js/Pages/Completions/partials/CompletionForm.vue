...
const props = defineProps({
    completion: Object, // Optional prop for editing an existing completion
});

// Initialize form with completion data if provided
const form = useForm({
    model: props.completion?.model || '',
    name: props.completion?.name || '',
    description: props.completion?.description || '',
    instructions: props.completion?.instructions || '',
});
...
// Determine endpoint based on presence of completion
const submitForm = () => {
    errorDetails = null;

    const endpoint = props.completion
        ? route('completions.update', props.completion.id)
        : route('completions.create');

    const method = props.completion ? 'put' : 'post';
...
<template>
    <form @submit.prevent="submitForm" class="space-y-4 p-4 bg-white dark:bg-gray-800 rounded shadow">
        <h3 class="text-lg font-bold mb-4 text-gray-800 dark:text-gray-200">
            {{ props.completion ? 'Update completion' : 'Create completion' }}
        </h3>
        ...
        <div>
            <InputLabel for="name" value="Completion Name" />
            <TextInput id="name" v-model="form.name" placeholder="Completion Name" class="w-full" />
        ...
        <div>
            ...
            <TextAreaInput
                ...
                placeholder="Provide completion description"
                rows="4"
                class="w-full"
            />
        ...
        <div>
            ...
            <TextAreaInput
                ...
                placeholder="Provide completion instructions"
                rows="8"
                class="w-full"
            />
            <p class="text-sm text-gray-700 dark:text-gray-300">
                {{ 60000 - form.instructions.length }} Characters remaining
            </p>
